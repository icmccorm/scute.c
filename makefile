...
EM_JS_FLAGS = $(EM_JS_EXPORTS) -s SAFE_HEAP=1 -s ALIASING_FUNCTION_POINTERS=0 -s ASSERTIONS=2 -s WASM=1 -s MODULARIZE=1 -s STRICT=1 -s FILESYSTEM=0 -s EXPORT_ES6=1 -s USE_ES6_IMPORT_META=0 -s ENVIRONMENT='worker' -s EXPORT_NAME="'Scute'"
...
emcc : $(SRC_FILES) $(EM_ENTRY)
	@$(WASMC) $(EM_MAP) $(EM_FLAGS) $(EM_JS_FLAGS) -g4 $(INC_FLAGS) -D EM_MAIN $(EM_ENTRY) $(SRC_FILES) -o ./$(EXEC_FILE).js 